...
const fetcher = (url, opts) => fetch(`${API}${url}`, opts).then(r => r.json());
...
const IconMsg     = () => <Icon d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetcher("/api/status").then(s => setStatus(s.status)).catch(() => {});
    }, 10000);
  ...
  const agregarContacto = async () => {
    ...
    const nuevo = await fetcher("/api/contactos", {
      ...
      body: JSON.stringify({ nombre: nuevoNombre.trim(), numero: nuevoNumero.trim() }),
    });
  ...
  const enviarAhora = async () => {
    if (status !== "listo") return showToast("El bot no está conectado", "error");
    setEnviando(true);
  ...
  const statusColor = { listo: "#22c55e", esperando_qr: "#f59e0b", desconectado: "#ef4444" }[status] || "#ef4444";
  const statusLabel = { listo: "Conectado", esperando_qr: "Esperando QR", desconectado: "Desconectado" }[status] || status;

  return (
    <div style={styles.root}>
      {/* TOAST */}
      {toast && (
        <div style={{ ...styles.toast, background: toast.tipo === "error" ? "#ef4444" : "#166534" }}>
        ...
        {tab === "contactos" && (
          <div style={styles.section}>
            <div style={styles.sectionHeader}>
              <h2 style={styles.sectionTitle}>Contactos <span style={styles.badge}>{contactos.length}</span></h2>
            </div>

            {/* Agregar */}
            <div style={styles.addBox}>
              <input style={styles.input} placeholder="Nombre" value={nuevoNombre}
                onChange={e => setNuevoNombre(e.target.value)} />
              <input style={styles.input} placeholder="Número (ej: 5492346567607)" value={nuevoNumero}
            ...
            <div style={styles.hint}>
              Formato: código país + código área sin 0 + número. Ejemplo: <code style={styles.code}>5492346123456</code>
            </div>

            {/* Lista */}
            <div style={styles.list}>
              {contactos.length === 0 && (
                <div style={styles.empty}>Sin contactos. Agregá el primero arriba.</div>
              )}
              {contactos.map(c => (
                <div key={c.id} style={styles.listItem}>
                  ...
                  <button style={styles.btnDanger} onClick={() => eliminarContacto(c.id)}>
                    <IconTrash />
                  </button>
                </div>
              ))}
        ...
        {tab === "mensaje" && (
          <div style={styles.section}>
            <div style={styles.sectionHeader}>
              ...
              {!editando && (
                <button style={styles.btnOutline} onClick={() => setEditando(true)}>
                  ✏️ Editar
                </button>
              )}
            </div>
            ...
            {editando ? (
              <>
                <textarea style={styles.textarea} value={mensajeEdit}
                  onChange={e => setMensajeEdit(e.target.value)} rows={10} />
                <div style={styles.hint}>
                  Usá <code style={styles.code}>{"{{fecha}}"}</code> y se reemplazará automáticamente con la fecha del envío.
                </div>
                <div style={styles.btnRow}>
                  <button style={styles.btnGhost} onClick={() => { setEditando(false); setMensajeEdit(mensaje); }}>
                    Cancelar
                  </button>
                  <button style={styles.btnGreen} onClick={guardarMensaje} disabled={guardando}>
            ...
            <div style={styles.sendBox}>
              <div>
                ...
                <div style={styles.sendSub}>El envío automático ocurre el día 27 de cada mes a las 10:00 hs.</div>
              </div>
              <button style={{
                ...styles.btnSend,
                opacity: status !== "listo" || enviando ? 0.5 : 1,
                cursor: status !== "listo" || enviando ? "not-allowed" : "pointer",
              }} onClick={enviarAhora} disabled={status !== "listo" || enviando}>
              ...
              <div key={h.id} style={styles.histCard}>
                <div style={styles.histHeader}>
                  ...
                  <div style={styles.histStats}>
                    <span style={{ color: "#22c55e" }}>✓ {h.enviados.length}</span>
                    {h.fallidos.length > 0 && (
                      <span style={{ color: "#ef4444", marginLeft: 8 }}>✗ {h.fallidos.length}</span>
                    )}
                  </div>
                </div>
                {h.enviados.length > 0 && (
                  <div style={styles.histGroup}>
                    ...
                    {h.enviados.map((e, i) => (
                      <div key={i} style={styles.histRow}>
                        <span style={{ color: "#22c55e" }}>✓</span> {e.nombre} <span style={styles.histNum}>+{e.numero}</span>
                      </div>
                    ))}
                  </div>
                )}
                {h.fallidos.length > 0 && (
                  <div style={styles.histGroup}>
                    <div style={{ ...styles.histGroupLabel, color: "#ef4444" }}>Fallidos</div>
                    {h.fallidos.map((f, i) => (
                      <div key={i} style={styles.histRow}>
                        <span style={{ color: "#ef4444" }}>✗</span> {f.nombre} — <span style={styles.histNum}>{f.error}</span>
                      </div>
                    ))}
                  </div>
                )}
...
const styles = {
  ...
  statusBadge: {
    ...
    background: "#1e2535", padding: "6px 12px", borderRadius: 20,
  },
  ...
  sectionTitle: { margin: 0, fontSize: 18, fontWeight: 700, color: "#f1f5f9", display: "flex", alignItems: "center", gap: 8 },
  badge: {
  ...
  code: { background: "#1e2535", padding: "1px 6px", borderRadius: 4, fontFamily: "monospace" },
  list: { display: "flex", flexDirection: "column", gap: 8 },
  listItem: {
    display: "flex", alignItems: "center", gap: 12,
    background: "#1e2535", padding: "12px 16px", borderRadius: 10,
  },
  avatar: {
    width: 38, height: 38, borderRadius: "50%",
    background: "#16423c", color: "#22c55e",
  ...
  empty: { color: "#64748b", textAlign: "center", padding: "30px 0", fontSize: 14 },
  textarea: {
    width: "100%", padding: "14px", background: "#1e2535",
  ...
  msgPreview: {
    background: "#1e2535", padding: "16px", borderRadius: 10,
  ...
  sendBox: {
    marginTop: 8,
    background: "#1e2535", padding: "18px", borderRadius: 12,
    display: "flex", alignItems: "center", justifyContent: "space-between",
  ...
  btnSend: {
    display: "flex", alignItems: "center", gap: 8,
    padding: "12px 22px", background: "#15803d", border: "none",
    borderRadius: 10, color: "#fff", fontWeight: 700,
    fontSize: 14, transition: "all .2s",
  },
  histCard: {
    background: "#1e2535", borderRadius: 12, padding: "16px",
  ...
  histGroupLabel: { fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: 1 },
  histRow: { fontSize: 13, color: "#94a3b8", display: "flex", gap: 6, alignItems: "center" },
